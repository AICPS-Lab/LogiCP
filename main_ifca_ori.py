...
def main():
    ...
    if "train" in args.mode and args.cluster > 0:   
        ...
        save_model(model_path, glob_model, str(args.dataset)+"_"+str(args.model)+"_IFCA"+"{}".format(args.client), ix_epoch)
        for c in range(args.cluster):
            glob_model.load_state_dict(cluster_weights[c])
            save_model(model_path, glob_model, str(args.dataset)+"_"+str(args.model)+"_IFCA_Cluster_"+str(c)+"_"+"{}".format(args.client), ix_epoch)
        
        for c in range(args.client):
            glob_model.load_state_dict(local_weights[c])
            save_model(model_path, glob_model, str(args.dataset)+"_"+str(args.model)+"_IFCA_Client_"+str(c)+"_"+"{}".format(args.client), ix_epoch)
