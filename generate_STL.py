...
def generate_property_test(tensor_arr, property_type = "upper", mining_range = 2):
    ...
    # print(tensor_arr.shape) # [5, 120]
    # exit(0)
    
    stlsyn_lib = []
    for j in range(tensor_arr.shape[1]): # 120
    ...
    if property_type == "upper":
        ...
        for temp_idx, templ in enumerate(STL_templates[property_type]):
            conf_interval_dict = {
                'x1': conf_interval[:,1].cuda().numpy(), # lower
                'x2': conf_interval[:,2].cuda().numpy(), # upper
            }
        ...
        for temp_idx, templ in enumerate(STL_templates[property_type]):
            try:
                (stlsyn, value, dur) = synth.synthSTLParam(templ, conf_interval[:,2].cuda().numpy(), optmethod)
                property[:, mining_range*temp_idx:mining_range*temp_idx+mining_range] = stlsyn.subformula.bound
            except:
        ...
        for temp_idx, templ in enumerate(STL_templates[property_type]):
            try:
                (stlsyn, value, dur) = synth.synthSTLParam(templ, conf_interval[:,1].cuda().numpy(), optmethod)
                property[:, mining_range*temp_idx:mining_range*temp_idx+mining_range] = stlsyn.subformula.bound
            except:
...
def get_robustness_score(tensor_arr, pred, property_type = "upper", mining_range = 2):
    ...
    if property_type == "upper":
        ...
        (stlsyn, value, dur) = synth.synthSTLParam(STL_templates[property_type][0], conf_interval[:,1].cuda().numpy(), optmethod)
        property_list.append(stlsyn.subformula.bound)
        (stlsyn, value, dur) = synth.synthSTLParam(STL_templates[property_type][1], conf_interval[:,2].cuda().numpy(), optmethod)
        property_list.append(stlsyn.subformula.bound)  
            
    
    # get robustness score
    prop_score = torch.zeros(1)
...
def generate_property(tensor_arr, property_type = "corr", mining_range = 2):
    ...
    if property_type == "upper":
        ...
        for temp_idx, templ in enumerate(STL_templates[property_type]):
            val_dict = {
                'x1': conf_interval[:,2].cuda().numpy(),
                'x2': conf_interval_guide[:,2].cuda().numpy(),
            }
            (stlsyn, value, dur) = synth.synthSTLParam(templ, [val_dict], optmethod)
            property[:, mining_range*temp_idx:mining_range*temp_idx+mining_range] = stlsyn.subformula.bound
        ...
        for temp_idx, templ in enumerate(STL_templates[property_type]):
            val_dict = {
                'x1': conf_interval[:,1].cuda().numpy(),
                'x2': conf_interval_guide[:,1].cuda().numpy(),
            }
            (stlsyn, value, dur) = synth.synthSTLParam(templ, [val_dict], optmethod)
            property[:, mining_range*temp_idx:mining_range*temp_idx+mining_range] = stlsyn.subformula.bound
    # cpu
