...
import numpy as np
import torch
from utils.update import LocalUpdate, compute_cluster_id, dic_loader_without_specm, dic_loader
from utils_training import get_device, to_device, save_model, get_shared_dataset, model_init, save_cp_result_with_sep_type, get_shared_dataset_2
import sys
import os
import copy
...
def main():
    ...
    for c in range(args.client):
        ...
        client_dataset[c]["nor"] = nor_loader_private # dataloader for the test data
    ...
    assert args.sep_type == "value"
    assert args.method == "IFCA"
   ...
   ############################
    ...
    if args.mode == "pretrain_calib":
        ...
        print(f"==========================================")
        print("Loaded vanilla IFCA client models.")
        print(f"==========================================")
        ...
        if args.dataset == "fhwa":
            model_path = f"hdd_{args.method}/saved_pretrain_models_{args.sep_type}_only/"
        elif args.dataset == "ct":
            model_path = f"{args.dataset}_{args.method}/saved_pretrain_models_{args.sep_type}_only/"
        print(model_path)
    ...
    # training with clusters
        ...
        for ix_epoch in range(1, epoch_num+1):
            ...
            client2cluster = {client: group for group, clients in cluster_id.items() for client in clients}                            # client:  cluster
        ...
        if args.dataset == "fhwa":
            model_path = f"hdd_{args.method}/saved_models/"
        glob_model.load_state_dict(glob_weight)
        ...
        for c in range(args.cluster):
            ...
            save_model(model_path, glob_model, str(args.dataset)+"_"+str(args.model)+"_IFCA_Cluster_"+str(c)+'_'+str(args.client), ix_epoch)
        
        for c in range(args.client):
            glob_model.load_state_dict(local_weights[c])
            save_model(model_path, glob_model, str(args.dataset)+"_"+str(args.model)+"_IFCA_Client_"+str(c)+'_'+str(args.client), ix_epoch)
